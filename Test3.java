...
public class TestGenerator {
    ...
    private static void addTestMethods(TypeSpec.Builder testClass, CompilationUnit cu, String className, List<MethodCallInfo> methodCalls) {
        cu.findAll(MethodDeclaration.class).stream()
                ...
                .forEach(method -> {
                    testClass.addMethod(buildTestMethod(method, className, methodCalls));
                });
    }

    private static MethodSpec buildTestMethod(MethodDeclaration method, String className, List<MethodCallInfo> methodCalls) {
        ...
        methodCalls.stream()
                .filter(mc -> mc.calledWithin.equals(methodName))
    ...
    private static void generateMockSetup(CodeBlock.Builder code, MethodCallInfo call) {
        code.addStatement("$T.when($L.$L($L)).thenReturn($T.any())",
                ClassName.get("org.mockito", "Mockito"), call.scope, call.methodName,
